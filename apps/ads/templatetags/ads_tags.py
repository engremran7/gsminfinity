...
def render_ad_slot(
    context, slug: str, allowed_types: str = "", allowed_sizes: str = ""
):
    """
    ...
    try:
        placement = AdPlacement.objects.filter(
            slug=slug, is_active=True, is_enabled=True, is_deleted=False
        ).first()
        html = render_to_string(
            "ads/components/slot.html",
            {
                ...
                or getattr(placement, "allowed_types", ""),
                "allowed_sizes": allowed_sizes
                or getattr(placement, "allowed_sizes", ""),
    ...
    try:
        if network_type == "adsense" and settings_obj.adsense_enabled:
            ...
            network = AdNetwork.objects.filter(
                network_type=network_type, is_enabled=True
            ).first()
            if network:
...
def ads_header_scripts(context):
    ...
    try:
        ...
        for network in AdNetwork.objects.filter(is_enabled=True).exclude(
            header_script=""
        ):
            networks.append(
                {
...
def render_affiliate_products(
    ...
    try:
        products = _get_contextual_products(
            ...
            blog_post=blog_post,
            max_products=min(
                max_products, settings_obj.affiliate_products_max_per_page
            ),
...
def _get_contextual_products(
    ...
    manual_matches = (
        AffiliateProductMatch.objects.filter(
            match_filters,
            product__is_enabled=True,
            product__is_in_stock=True,
        )
        .select_related("product", "product__provider")
        .order_by("-is_pinned", "position", "-relevance_score")[:max_products]
    )
    ...
    if brand:
        ...
        targeted_filters &= Q(target_models=variant.model) | Q(
            target_brands=variant.model.brand
        )
    ...
    if brand:
        ...
        keywords.extend(
            [model.name.lower(), model.brand.name.lower() if model.brand else ""]
        )
    elif variant and hasattr(variant, "model") and variant.model:
...
@register.inclusion_tag(
    "ads/components/affiliate_product_card.html", takes_context=True
)
