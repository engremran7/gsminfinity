    ...
    def post_after_save(sender, instance, created=False, **kwargs):
        ...
        if created and is_published and notifications_enabled():
            try:
                ...
                logger.warning(f"Failed to queue notification task for post {instance.pk}: {e}")
        ...
        if created and is_published:
            try:
                ...
                if sitemap_url.startswith("http"):
                    ...
                    logger.info(f"Queued search engine ping task for post {instance.pk}")
            except Exception as e:
                logger.warning(f"Failed to queue search engine ping for post {instance.pk}: {e}")
        ...
        if instance.status == PostStatus.PUBLISHED:
            try:
                ...
                logger.warning(f"Failed to queue auto-translation for post {instance.pk}: {e}")
