...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING(f'DRY RUN MODE - No data will be deleted'))
        
        self.stdout.write(self.style.SUCCESS(f'Cleaning up data older than {days} days ({cutoff.date()})...'))
    ...
    def cleanup_sessions(self, dry_run=False):
        ...
        try:
            ...
            if count > 0:
                if not dry_run:
                    expired.delete()
                self.stdout.write(f'  Sessions: {count:,} expired sessions {"would be" if dry_run else ""} deleted')
                return count
            else:
    ...
    def cleanup_download_sessions(self, cutoff, dry_run=False):
        ...
        try:
            ...
            if count > 0:
                if not dry_run:
                    old_sessions.delete()
                self.stdout.write(f'  Download Sessions: {count:,} old sessions {"would be" if dry_run else ""} deleted')
                return count
            else:
    ...
    def cleanup_page_views(self, cutoff, dry_run=False):
        ...
        try:
            ...
            if count > 0:
                if not dry_run:
                    old_views.delete()
                self.stdout.write(f'  Page Views: {count:,} old records {"would be" if dry_run else ""} deleted')
                return count
            else:
    ...
    def cleanup_events(self, cutoff, dry_run=False):
        ...
        try:
            ...
            if count > 0:
                if not dry_run:
                    old_events.delete()
                self.stdout.write(f'  Events: {count:,} old records {"would be" if dry_run else ""} deleted')
                return count
            else:
    ...
    def cleanup_ad_events(self, cutoff, dry_run=False):
        ...
        try:
            ...
            if count > 0:
                if not dry_run:
                    old_events.delete()
                self.stdout.write(f'  Ad Events: {count:,} old records {"would be" if dry_run else ""} deleted')
                return count
            else:
