...
class FirmwareBlogService:
    ...
    def ensure_model_category(cls, model):
        ...
        try:
            ...
            if created:
                logger.info(f"Created blog category for model: {model.name} under {model.brand.name}")
    ...
    def generate_firmware_post(cls, model, force_update=False):
        ...
        try:
            ...
            if existing_post:
                ...
                existing_post.category = category  # Update category with new parent structure
            ...
            category = cls.ensure_model_category(model)
            brand_category = cls.ensure_brand_category(model.brand)
    ...
    def _get_firmware_storage_info(cls, firmware):
        ...
        try:
            ...
            if locations.exists():
                ...
                storage_info['download_count'] = sum(loc.download_count for loc in locations)
    ...
    def _generate_post_body(cls, model, firmware_data):
        ...
        # Introduction
        html_parts.append(f'<h2>Download {model.brand.name} {model.name} Firmware - All Variants</h2>')
        html_parts.append(f'<p>Complete collection of firmware files for <strong>{model.brand.name} {model.name}</strong>. ')
        html_parts.append(f'We have <strong>{firmware_data["total_files"]} firmware files</strong> available across ')
        html_parts.append(f'<strong>{len(firmware_data["variants"])} variants</strong>.</p>')
        ...
        html_parts.append('📢 <strong>Auto-Updated Post:</strong> This firmware list is automatically updated ')
        html_parts.append('whenever new files are uploaded. Check back regularly for the latest versions.')
        ...
        for variant_data in firmware_data['variants']:
            ...
            html_parts.append(f'<h3>📱 {variant.name} ({variant.region or "Global"})</h3>')

            if variant.board_id:
                html_parts.append(f'<p><strong>Board ID:</strong> <code>{variant.board_id}</code></p>')

            html_parts.append(f'<p>Found <strong>{len(firmwares)} firmware file(s)</strong> for this variant:</p>')
            ...
            for fw_data in firmwares:
                ...
                html_parts.append(f'<td><span class="badge badge-primary">{fw_data["type"]}</span></td>')
                ...
                if not storage_info['is_uploaded']:
                    html_parts.append('<td><span class="badge badge-warning">⏳ Processing</span></td>')
                elif fw_data['is_password_protected']:
                    html_parts.append('<td><span class="badge badge-info">🔒 Password Protected</span></td>')
                else:
                    html_parts.append('<td><span class="badge badge-success">✅ Ready</span></td>')
                ...
                if storage_info['is_uploaded']:
                    ...
                    html_parts.append(f'<td><a href="{download_url}" class="btn btn-sm btn-primary">Download</a></td>')
                else:
                    html_parts.append('<td><button class="btn btn-sm btn-secondary" disabled>Processing...</button></td>')
        ...
        html_parts.append('<li>Ensure you download the correct firmware for your device variant</li>')
        html_parts.append('<li>Check the chipset compatibility before flashing</li>')
        html_parts.append('<li>Password-protected files require extraction password (contact support)</li>')
        html_parts.append('</ul>')

        html_parts.append(f'<p><small>Last updated: {timezone.now().strftime("%B %d, %Y")}</small></p>')
    ...
    def _generate_summary(cls, model, firmware_data):
        ...
        summary = f'Download {model.brand.name} {model.name} firmware for all variants. '
    ...
    def _update_post_tags(cls, post, model, firmware_data):
        ...
        try:
            ...
            try:
                if apps.is_installed('apps.distribution'):
                    DistributionSettings = apps.get_model('distribution', 'DistributionSettings')
            ...
            brand_tag, _ = Tag.objects.get_or_create(
                slug=slugify(model.brand.name),
                defaults={'name': model.brand.name}
            )
            ...
            for chipset in list(chipsets)[:3]:  # Limit to 3 chipsets
                chipset_tag, _ = Tag.objects.get_or_create(
                    slug=slugify(chipset),
                    defaults={'name': chipset}
                )
            ...
            logger.info(f"Added {len(tags_to_add)} tags to post including {len(chipsets)} chipsets (max allowed: {MAX_AUTO_TAGS} - configurable in admin)")
    ...
    def _update_post_seo(cls, post, model, firmware_data):
        ...
        try:
            ...
            logger.info(f"Updated SEO (title: {len(post.seo_title)}/{MAX_SEO_TITLE}, desc: {len(post.seo_description)}/{MAX_SEO_DESC})")
    ...
    def _create_distribution_plan(cls, post, model, firmware_data):
        ...
        try:
            ...
            if not settings.enable_firmware_auto_distribution:
                logger.info(f"Firmware auto-distribution disabled - skipping: {post.title}")
                return
            ...
            target_channels = settings.default_channels[:MAX_PLATFORMS] if settings.default_channels else [
            ...
            distribution, created = ContentDistribution.objects.update_or_create(
                ...
                defaults={
                    ...
                    'summary': post.summary[:settings.max_seo_description_length] if post.summary else '',
            ...
            logger.info(f"{'Created' if created else 'Updated'} distribution plan: {post.title} ({len(target_channels)} platforms)")
