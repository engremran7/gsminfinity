...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        brand_prefixes = [
            ...
            'Motorola', 'Nokia', 'Sony', 'LG', 'HTC', 'Asus', 'Lenovo', 'ZTE',
            'Meizu', 'Infinix', 'Tecno', 'iTel', 'Gionee', 'Coolpad', 'LeEco',
            'Poco', 'Redmi', 'Honor', 'iQOO', 'Nothing', 'Micromax', 'Lava',
            'Karbonn', 'Intex', 'Xolo', 'Panasonic', 'Sharp', 'Fujitsu',
            'Alcatel', 'TCL', 'BLU', 'Wiko', 'Archos', 'Vertex', 'Doogee',
        ...
        model_series = [
            ...
            'P Series', 'Nova', 'Y Series', 'A Series', 'M Series', 'F Series',
        ...
        chipsets = [
            ...
            'Snapdragon 780G', 'Snapdragon 750G', 'Snapdragon 695', 'Snapdragon 480',
            'MediaTek Dimensity 9200', 'MediaTek Dimensity 8200', 'MediaTek Dimensity 7200',
            'MediaTek Dimensity 6020', 'MediaTek Helio G99', 'MediaTek Helio G96',
        ...
        for brand_slug, brand in brands.items():
            ...
            for i in range(num_models):
                ...
                for v in range(num_variants):
                    ...
                    if 'samsung' in brand_slug.lower() or 'galaxy' in model_name.lower():
                        variant_code = f"SM-{chr(65 + random.randint(0, 25))}{random.randint(100, 999)}{chr(65 + random.randint(0, 25))}"
                    elif 'xiaomi' in brand_slug.lower() or 'redmi' in brand_slug.lower():
        ...
        for idx, variant in enumerate(created_variants, 1):
            ...
            for i in range(num_firmwares):
                ...
                if rand_val <= 60:
                    firmware_type_name, FirmwareClass = 'official', OfficialFirmware
                    official_count += 1
                elif rand_val <= 80:
                    firmware_type_name, FirmwareClass = 'engineering', EngineeringFirmware
                    engineering_count += 1
                else:
                ...
                firmware_data = {
                    ...
                    'metadata': {
                        ...
                        'file_size_bytes': random.randint(1500000000, 6000000000),  # 1.5-6GB
                ...
                if FirmwareClass == ModifiedFirmware:
                    firmware_data['subtype'] = random.choice([
                        'Root', 'Custom Recovery', 'Debloated', 'Performance Mod',
                        'Camera Mod', 'Battery Optimization', 'Kernel', 'ROM'
                    ])
        ...
        for i, variant in enumerate(sample_variants):
            ...
            pending, created = PendingFirmware.objects.get_or_create(
                ...
                defaults={
                    ...
                    'chipset': chipset,
                    'admin_decision': random.choice(['pending', 'pending', 'pending', 'approved', 'rejected']),
                    'metadata': {
                        'version': f'V{random.randint(1, 15)}.{random.randint(0, 9)}.{random.randint(0, 99)}',
                        'uploaded_at': (timezone.now() - timedelta(days=random.randint(1, 60))).isoformat(),
                    },
                }
            )
            if created:
        ...
        for variant in sample_variants_for_requests:
            request, created = FirmwareRequest.objects.get_or_create(
                ...
                firmware_type=random.choice(['official', 'official', 'engineering', 'modified']),
                defaults={
                    ...
                    'status': random.choice(['open', 'open', 'open', 'in_progress', 'fulfilled']),
