...
def firmware_download_start(
    ...
    # Log download
    logger.info(
        f"Firmware download started: {firmware.original_file_name} by {request.user if request.user.is_authenticated else 'anonymous'}"
    )
    ...
    if firmware.stored_file_path:
        ...
        if firmware.stored_file_path.startswith(("http://", "https://")):
            ...
            messages.info(
                request, f"Download would start for: {firmware.original_file_name}"
            )
            return redirect(
            ...
            results.append(
                {
                    "type": "model",
                    "id": model.id,
                    "name": model.marketing_name
                    if model.marketing_name
                    else model.name,
                    "slug": model.slug,
                    "url": reverse(
