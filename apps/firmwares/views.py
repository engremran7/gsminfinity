...
class FirmwareUploadView(views.APIView):
    ...
    def post(self, request):
        ...
        model = Model.objects.filter(id=data.get("model_id")).first() if data.get("model_id") else None
        variant = Variant.objects.filter(id=data.get("variant_id")).first() if data.get("variant_id") else None
        try:
            pf = handle_upload(
        ...
        run_ai_analysis(pf)
        return Response(PendingFirmwareSerializer(pf).data, status=status.HTTP_201_CREATED)
