...
def resolve_locale(
    ...
    try:
        lang_cookie = request.COOKIES.get(
...
def resolve_key(
    ...
    tk = TranslationKey.objects.filter(
        app_id=app_id, namespace=namespace, key=key_name
    ).first()
    if not tk:
        if log_missing:
            MissingKeyLog.objects.create(
                app_id=app_id, site_id=site_id, locale=locale, key=key_name
            )
        return default or key

    # primary locale
    tv = TranslationValue.objects.filter(
        translation_key=tk, locale=locale, status="approved"
    ).first()
    if tv:
        return tv.message
    ...
    if fallback_locale:
        tv = TranslationValue.objects.filter(
            translation_key=tk, locale=fallback_locale, status="approved"
        ).first()
        if tv:
            return tv.message
    ...
    # global default
    tv = TranslationValue.objects.filter(
        translation_key=tk, locale="en", status="approved"
    ).first()
    if tv:
        return tv.message
...
def resolve_theme(
    ...
    if site_id:
        assignments = assignments.filter(
            models.Q(site_id=site_id) | models.Q(site_id__isnull=True)
        )
    ...
    assignment = (
        ...
        or (
            device_pref and _latest(assignments.filter(device_pref=device_pref))
        )  # device pref
        or (
            system_pref and _latest(assignments.filter(system_pref=system_pref))
        )  # system pref
    ...
    if not theme and desired_mode:
        theme = (
            Theme.objects.filter(
                app_id=app_id, site_id=site_id, mode=desired_mode
            ).first()
    ...
    try:
        font = (
            FontRegistry.objects.filter(
                is_default_for_locales__contains=[locale]
            ).first()
            or FontRegistry.objects.filter(
                is_default_for_locales__contains=[locale.split("-")[0]]
            ).first()
        )
        if font:
    ...
    result = {
        ...
        "mode": theme.mode,
        "tokens": tokens,
        "direction": tokens["typography"]["direction"],
    }
...
def register_manifest(
    ...
    with transaction.atomic():
        ...
        _audit(
            ...
            "manifest_updated",
            app_id,
            after={"namespaces": namespaces, "supported_locales": supported_locales},
        )
        return manifest
...
def _audit(
    ...
    try:
        AuditLog.objects.create(
            actor=actor, action=action, app_id=app_id, before=before or {}
        )
    except Exception:
        return
...
# ---------------------------------------------------------------------------
def auto_translate(
    app_id: str, key: TranslationKey, target_locale: str, provider: str | None = None
    ...
    try:
        ...
        source_value = TranslationValue.objects.filter(
            translation_key=key, locale__in=["en", "en-US"], status="approved"
        ).first()
        ...
        translated_text = translator.translate(
            text=source_value.message, source_lang="en", target_lang=target_locale
        )
        ...
        cache.delete_pattern(cache_key) if hasattr(
...
def auto_translate_batch(
    app_id: str, target_locale: str, namespace: str | None = None, limit: int = 100
) -> dict:
    """
