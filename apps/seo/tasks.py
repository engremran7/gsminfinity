...
def build_sitemap_async(self, sitemap_type: str = 'all', notify: bool = False):
    ...
    try:
                ...
                for model in models.iterator():
                    entry, created = SitemapEntry.objects.update_or_create(
                        ...
                        defaults={
                            'changefreq': 'daily',
                            'priority': 0.9,
                            'lastmod': model.updated_at if hasattr(model, 'updated_at') else timezone.now(),
                            'is_active': True,
                        }
                    )
                    if created:
                ...
                for tag in tags.iterator():
                    entry, created = SitemapEntry.objects.update_or_create(
                        ...
                        defaults={
                            'changefreq': 'weekly',
                            'priority': 0.6,
                            'lastmod': tag.updated_at if hasattr(tag, 'updated_at') else timezone.now(),
                            'is_active': True,
                        }
                    )
                    if created:
        ...
        logger.info(f"Sitemap build complete: {entries_created} new entries for type '{sitemap_type}'")
...
def check_links_async(self, limit: int = 100):
    ...
    try:
        ...
        if not entries.exists():
            ...
            entries = SitemapEntry.objects.filter(is_active=True).order_by('last_checked')[:limit]
        ...
        for entry in entries:
            try:
                ...
                if response.status_code >= 400:
                    results['broken'] += 1
                    logger.warning(f"Broken link detected: {entry.url} (HTTP {response.status_code})")
                else:
                    results['ok'] += 1
        ...
        logger.info(f"Link check complete: {results['checked']} checked, {results['ok']} OK, {results['broken']} broken")
        return results
        ...
        if ct.app_label == 'blog' and ct.model == 'post':
            ...
            for obj in queryset.iterator():
                try:
                    ...
                    logger.error(f"SEO regeneration failed for {ct.model} {obj.pk}: {e}")

            logger.info(f"SEO regenerated for {count} {ct.model} objects")
            return {'regenerated': count}

        logger.warning(f"SEO regeneration not implemented for {ct.app_label}.{ct.model}")
