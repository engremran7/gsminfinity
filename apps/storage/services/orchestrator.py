...
from ..models import FirmwareStorageLocation, SharedDriveAccount, UserDownloadSession
...
class DownloadOrchestrator:
    ...
    def initiate_download(
        ...
        if existing:
            logger.info(f"Returning existing session {existing.id} for user {user.username}")
            return existing

        # Try Google Drive first
        gdrive_location = FirmwareStorageLocation.objects.filter(
            content_type=content_type,
            object_id=firmware_object.id,
            storage_type='shared_drive',
            is_primary=True
        ).select_related('shared_drive').first()
        ...
        external_location = FirmwareStorageLocation.objects.filter(
            content_type=content_type,
            object_id=firmware_object.id,
    ...
    def _initiate_gdrive_download(
        self,
        user,
        storage_location: FirmwareStorageLocation
    ) -> UserDownloadSession:
        ...
        service_account = self.router.get_best_service_account(
            ...
            preferred_drive_id=str(storage_location.shared_drive.id) if storage_location.shared_drive else None
        )
    ...
    def _create_external_download_session(
        ...
        session = UserDownloadSession.objects.create(
            ...
            status='ready',
            expires_at=storage_location.external_expiry or (
    ...
    def _fallback_to_external(
        self,
        user,
        gdrive_location: FirmwareStorageLocation
    ):
    ...
    def get_session_status(self, session_id: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                'download_link': session.user_gdrive_link if session.status == 'ready' else None,
                ...
                'copy_completed_at': session.copy_completed_at.isoformat() if session.copy_completed_at else None
            }
...
class UploadOrchestrator:
    ...
    def upload_firmware(
        ...
        service_account = self.router.get_best_service_account(
            required_gb=file_size_gb,
            preferred_drive_id=str(target_drive.id)
        )

        if not service_account:
            raise ValueError(f"No service accounts available with {file_size_gb:.2f}GB quota")
        ...
        parent_folder_id = self._ensure_folder_structure(
            gdrive_service, target_drive, brand_name, model_name, variant_name, category, folder_org
        )
    ...
    def _ensure_folder_structure(
        ...
        if variant_name:
            if folder_org and folder_org.variant_folder_id:
                ...
                variant_folder_id = gdrive_service.create_folder(variant_name, parent_id)
                if folder_org:
