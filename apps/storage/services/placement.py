...
from ..models import SharedDriveAccount, DriveFileOrganization, FirmwareStorageLocation
import logging
...
class SmartPlacementAlgorithm:
    ...
    def select_optimal_drive(
        ...
        existing_org = DriveFileOrganization.objects.filter(
        ...
        if existing_org:
            ...
            if drive.can_accept_files(file_count):
                ...
                logger.warning(f"Brand exists in {drive.name} but drive is full, finding alternative")
        
        # Step 2: Find drive with available capacity
        available_drives = SharedDriveAccount.objects.filter(
            is_active=True,
            health_status__in=['healthy', 'warning']
        ...
        logger.error(f"No suitable drive found for brand={brand_name}, file_count={file_count}")
        return None
    ...
    def record_file_deletion(
        ...
        """
        if storage_location.storage_type != 'shared_drive' or not storage_location.shared_drive:
            return
        ...
        shared_drive.total_size_gb = max(0, shared_drive.total_size_gb - storage_location.file_size_gb())
        shared_drive.save(update_fields=['current_file_count', 'total_size_gb'])
    ...
    def get_brand_distribution(self) -> list[dict]:
        ...
        orgs = DriveFileOrganization.objects.filter(
            model_name='',  # Root brand level
            variant_name='',
            category_name=''
        ).select_related('shared_drive').order_by('brand_name')
    ...
    def suggest_migration(self, from_drive_id: str, to_drive_id: str, brand_name: str) -> dict:
        """
