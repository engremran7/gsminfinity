    ...
    def handle(self, *args, **options):
        ...
        for provider_id in expected_providers:
            ...
            if app and app.client_id and not app.client_id.startswith("PLACEHOLDER"):
                ...
                self.stdout.write(
                    self.style.WARNING(f"  ✗ {provider_id.capitalize()}: Not configured")
                )
        ...
        if configured:
            self.stdout.write(
                self.style.SUCCESS(f"✓ {len(configured)} provider(s) ready: {', '.join(configured)}")
            )
            self.stdout.write("  Social login buttons will appear on login/signup pages.")
        
        if missing:
            self.stdout.write("")
            self.stdout.write(
                self.style.WARNING(f"⚠ {len(missing)} provider(s) need configuration: {', '.join(missing)}")
            )
            self.stdout.write("")
            self.stdout.write("  To enable social login, add OAuth credentials in Django Admin:")
            ...
            self.stdout.write("  • Microsoft: https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps")
            self.stdout.write("  • GitHub:    https://github.com/settings/developers")
        
        if not missing:
            self.stdout.write("")
            self.stdout.write(self.style.SUCCESS("✓ All social providers are configured!"))
